...
class _KioskDashboardState extends State<KioskDashboard> {
      ...
      children: [
        ...
        Container(
          ...
          child: StreamBuilder<DatabaseEvent>(
            stream: FirebaseDatabase.instance.ref('walk_ins')
  ...
  Future<void> _handleWalkInSubmission(String reason) async {
    ...
    final activeTicketQuery = await FirebaseDatabase.instance.ref('walk_ins')
    ...
    if (activeTicketQuery.snapshot.exists) {
      ...
      var activeTickets = map.values.where((v) {
  ...
  Future<void> _generateWalkInTicket({required String name, required String id, required String reason, String? phone, String? email}) async {
    ...
    try {
      DateTime now = DateTime.now();
      DateTime startOfToday = DateTime(now.year, now.month, now.day);
      var snapshotEvent = await FirebaseDatabase.instance.ref('walk_ins')
          .orderByChild('timestamp')
          .startAt(startOfToday.millisecondsSinceEpoch)
          .once();

      int count = snapshotEvent.snapshot.exists ? (snapshotEvent.snapshot.value as Map).length : 0;
      queueNumber = 1000 + count + 1; 
      ...
      await FirebaseDatabase.instance.ref('walk_ins').push().set(walkInData);
      success = true;
  ...
  Future<void> _preCheckAppointment(String dept) async {
    var q = await FirebaseDatabase.instance.ref('appointments').orderByChild('patient_id').equalTo(widget.userId).once();
    bool hasActive = false;
    if (q.snapshot.exists) {
      var map = q.snapshot.value as Map<dynamic, dynamic>;
      hasActive = map.values.any((v) => v['status'] == 'Booked');
    }
    if (hasActive) {
      if (!mounted) return;
      showDialog(
        ...
        Expanded(
          child: StreamBuilder<DatabaseEvent>(
            ...
            builder: (context, snapshot) {
              ...
              var dataMap = snapshot.data!.snapshot.value as Map<dynamic, dynamic>;
              var docs = dataMap.entries.map((e) {
                var map = Map<String, dynamic>.from(e.value as Map);
                map['id'] = e.key;
                return map;
              }).toList();
              docs.sort((a, b) => (b['timestamp'] ?? 0).compareTo(a['timestamp'] ?? 0));
  ...
  Future<void> _cancelAppt(String id) async {
    await FirebaseDatabase.instance.ref('appointments').child(id).update({'status': 'Cancelled'});
  }
...
class _AppointmentPageState extends State<AppointmentPage> {
  ...
  Future<void> _checkExistingBookings() async {
    ...
    if (q.snapshot.exists) {
      var map = q.snapshot.value as Map<dynamic, dynamic>;
      active = map.values.any((v) => v['status'] == 'Booked');
    }
  ...
  Future<void> _fetchBookedSlots(DateTime d) async {
    ...
    var q = await FirebaseDatabase.instance.ref('appointments').orderByChild('date').equalTo(s).once();
    List<String> b = [];
    if (q.snapshot.exists) {
      var map = q.snapshot.value as Map<dynamic, dynamic>;
      b = map.values
  ...
  void _sub() async {
    ...
    Map<String, dynamic> data = {'department': widget.department, 'date': d, 'time': selTime, 'status': 'Booked', 'patient_name': widget.userName, 'patient_id': widget.userId, 'timestamp': ServerValue.timestamp};
    
    await FirebaseDatabase.instance.ref('appointments').push().set(data);
  ...
  List<String> _generateSlots(DateTime? d) {
    ...
    while (t.hour < 17) { 
      bool add = false;
      if (d.weekday == 5) {
        if (t.hour >= 8 && (t.hour < 12 || (t.hour == 12 && t.minute <= 15))) {
          add = true;
        }
      } else {
      ...
      if (add) s.add(DateFormat('hh:mm a').format(t)); 
