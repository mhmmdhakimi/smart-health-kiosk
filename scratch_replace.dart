  ...
  for (final file in files) {
    ...
    if (content.contains('MaterialPageRoute')) {
      ...
      content = content.replaceAllMapped(
        RegExp(r'MaterialPageRoute\s*\(\s*builder:\s*\([^)]+\)\s*=>\s*([^,)]+)\s*\)'),
        (m) => 'NoAnimRoute(page: ${m.group(1)})'
      );
      
      // Pattern 2: MaterialPageRoute(settings: settings, builder: (X) => Y)
      content = content.replaceAllMapped(
        RegExp(r'MaterialPageRoute\s*\(\s*settings:\s*(settings),\s*builder:\s*\([^)]+\)\s*=>\s*([^,)]+)\s*\)'),
        (m) => 'NoAnimRoute(settings: ${m.group(1)}, page: ${m.group(2)})'
      );

      // Add import
      String importStatement = '';
      if (file.path.contains('screens') || file.path.contains('widgets') || file.path.contains('core')) {
